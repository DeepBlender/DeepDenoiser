...
class RefinementNet:
  ...
  def __init__(self, number_of_repetitions=3, number_of_blocks=4, number_of_convolutions_per_block=2, number_block_repetitions=2, number_of_temporary_data_filters=0, number_of_filters_per_convolution=32, activation_function=tf.nn.relu, use_zero_padding=True, use_channel_weighting=False):
    ...
    self.number_of_temporary_data_filters = number_of_temporary_data_filters
  ...
  def __convolution_block(self, inputs, number_of_output_filters, convolution_block_index, reuse, is_training, data_format):
  ...
  def refinement_net(self, prediction_inputs, auxiliary_inputs, is_training, data_format='channels_last'):
    ...
    number_of_output_channels = intermediate_prediction.shape[concat_axis].value + self.number_of_temporary_data_filters
    
    temporary_data = None
    if self.number_of_temporary_data_filters > 0:
      # Warmup for temporary data.
      inputs = tf.concat([intermediate_prediction, prediction_inputs, auxiliary_inputs], concat_axis)
      temporary_data = self.__convolution_block(inputs, self.number_of_temporary_data_filters, 0, False, is_training, data_format)
        ...
        for j in range(self.number_of_blocks):
          ...
          for k in range(1 + self.number_block_repetitions):
            ...
            if self.number_of_temporary_data_filters == 0:
              ...
              inputs = tf.concat([intermediate_prediction, prediction_inputs, auxiliary_inputs, temporary_data], concat_axis)
            intermediate_prediction_delta = self.__convolution_block(inputs, number_of_output_channels, j + 1, reuse or reuse_block, is_training, data_format)
            if self.number_of_temporary_data_filters > 0:
              size_splits = [(number_of_output_channels - self.number_of_temporary_data_filters), self.number_of_temporary_data_filters]
              intermediate_prediction_delta, temporary_data = tf.split(intermediate_prediction_delta, size_splits, concat_axis)
